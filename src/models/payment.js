...
module.exports = {
    getPayment:  (sortBy = "id", asc = 1, page = 1, perPage = 5) =>{
      return new Promise(async(resolve, reject)=>{
        if(sortBy || page || perPage){
          ...
          match.previous_page = page>0? "http://localhost:3001/api/v1/payment?page="+ prevPage: null
          match.next_page = "http://localhost:3001/api/v1/payment?page="+ parseInt(parseInt(page)+1)  
  ...
  deletePayment: (id_payment) => {
    return new Promise((resolve, reject) => {
        connection.query("DELETE FROM payment WHERE id=?", id_payment, (err, result) => {
            if (!err) {
                resolve(result);
            } else {
                reject(new Error(err));
